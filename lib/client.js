  ...
  this.get(tableName, get, function (err, result) {
    ...
    if (kvs.length > 0) {
      ...
      for (var i = 0; i < kvs.length; i++) {
        var kv = kvs[i];
        //if there is invalid utf char in qualifier, convert it to base64. Eg. new Buffer([128])
        var qualifier = (new Buffer(kv.getQualifier().toString()).length === kv.getQualifier().length)
          ? kv.getQualifier().toString()
          : kv.getQualifier().toSTring("base64");
        r[kv.getFamily().toString() + ':' + qualifier] = kv.getValue();
      }
    }
    callback(null, r);
  });
